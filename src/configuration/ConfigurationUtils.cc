...
{
	mr->addAlias("platform:/plugin/com.tttech.tte.targetdevices/data/TTE_Prot_Layer_100M.device_targets", "TTE_Prot_Layer_100M.device_targets");
	mr->addAlias("platform:/plugin/com.tttech.tte.targetdevices/data/TTE_Dev_Switch_8port_100M_V2.device_targets", "TTE_Dev_Switch_8port_100M_V2.device_targets");
	mr->addAlias("platform:/plugin/com.tttech.tte.models/data/PredefinedIntegrityConfigurations.network_config", "PredefinedIntegrityConfigurations.network_config");
}
...
int ConfigurationUtils::getPortSerialNumber(System_Specification::Port_ptr port, Device_Target_Mapping::Mappings_ptr map){
		...
		if(!pms->eClass()->getName().compare("SwPortMappings")){
			...
			for(unsigned int j=0;j<portmapList.size();j++){
				...
				if(!pm->getRefPort()->getName().compare(port->getName())
						&& !pm->getRefTargetPort()->getName().find("PHY") != -1){
...
System_Specification::VirtualLink_ptr ConfigurationUtils::getVLfromVLid(int vlid, Device_Target_Mapping::Mappings_ptr map){
	Virtuallink_Map::VirtualLinkMapping_ptr vlming = map->getRefVirtualLinkMappings();
	ecorecpp::mapping::EList< Virtuallink_Map::VirtualLinkMap >& vlmList = vlming->getVirtualLinkMap();
	for(unsigned int j=0; j< vlmList.size(); j++){
		Virtuallink_Map::VirtualLinkMap_ptr vlm = vlmList.get(j);
