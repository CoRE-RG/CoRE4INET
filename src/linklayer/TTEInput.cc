...
void TTEInput::initialize()
{
	...
	DeviceSpecification *ds = ConfigurationUtils::getDeviceSpecification(getParentModule()->getParentModule()->par("device_name"),nc);
	assert(ds);
    ...
    if(msg->arrivedOn("in")){
    	...
    	if(isCT(frame)){
    		...
    		if(incomingList != incomings.end()){
    			...
    			for (std::list<Incoming*>::iterator incoming = incomingList->second.begin();
    					incoming != incomingList->second.end(); incoming++) {
...
bool TTEInput::isCT(EtherFrame *frame){
	...
	//Check for ct
	if((((macBytes[0]<<24)|(macBytes[1]<<16)|(macBytes[2]<<8)|(macBytes[3])) & ct_mask) == (ct_marker & ct_mask)){
		return true;
	}
	//TODO More checking ?
	return false;
